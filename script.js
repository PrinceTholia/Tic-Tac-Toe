...
class TicTacToe {
    ...
    // Method implementing the Minimax algorithm to find the best move
    minimax() {
        // Base cases: return values for win/loss/draw states
        if (this.isWin) return { value: -10 }; // Return a value of -10 if the game is won
        if (this.isDraw) return { value: 0 }; // Return a value of 0 if the game is drawn
        ...
        for (let move of this.validMoves) {
            ...
            let {value} = this.minimax(); // Recursively call minimax to evaluate the move
    ...
    goodMove() {
        let {moves} = this.minimax(); // Get equally valued moves from minimax
